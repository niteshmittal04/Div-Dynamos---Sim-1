...
async def ws_handler(websocket, path=None):
    ...
    try:
        # Send initial obstacle list to the simulator on connection
        await websocket.send(json.dumps({"command": "set_obstacles", "obstacles": current_obstacles}))
...
def get_obstacles():
    ...
    return jsonify({
        ...
        'count': len(current_obstacles),
        'message': 'Obstacle tracking is now handled server-side'
    })
...
def start_flask():
    global current_obstacles
    # Initialize obstacles on startup
    current_obstacles = generate_random_obstacles(8)
